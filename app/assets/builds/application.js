...
// app/javascript/controllers/close_form_controller.js
var close_form_controller_default = class extends Controller {
...
// app/javascript/controllers/reorder_controller.js
var reorder_controller_default = class extends Controller {
  static targets = ["item"];
  connect() {
  ...
  async updatePositions() {
    const response = await fetch("/todos/reorder", {
      ...
      body: JSON.stringify({ ordered_ids: this.orderedIds() })
    });
    if (!response.ok) {
...
// app/javascript/controllers/task_reorder_controller.js
var task_reorder_controller_default = class extends Controller {
  static targets = ["item"];
  connect() {
    this.draggedItem = null;
    this.originalOrder = this.orderedIds().join(",");
  ...
  dragOver(event) {
    event.preventDefault();
    const target = event.target.closest('[data-task-reorder-target="item"]');
    if (!target || target === this.draggedItem) return;
    const targetBox = target.getBoundingClientRect();
    const insertAfter = event.clientY > targetBox.top + targetBox.height / 2;
    const draggedWrapper = this.wrapperFor(this.draggedItem);
  ...
  async updatePositions() {
    ...
    const response = await fetch(`/todos/${todoId}/reorder_tasks`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        "X-CSRF-Token": document.querySelector("meta[name=csrf-token]").content
...
application.register("close-form", close_form_controller_default);
application.register("flash", flash_controller_default);
application.register("hello", hello_controller_default);
application.register("reorder", reorder_controller_default);
application.register("task-reorder", task_reorder_controller_default);
